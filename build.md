...
* Распакуйте архив, если он ещё не распакован. Архив не удаляйте.
* Войдите в редактирование скрипта установки ./Crypto/VinKekFish/build/arcs/dotnet7/exe/install.sh (запустите текстовый редактор, такой как xed, gedit, kate, geany).
* В файле скрипта необходимо поменять второй параметр командной строки на путь к файлу vkf-dotnet7.7z .
* Поменяйте прямо в архиве ./Crypto/VinKekFish/build/arcs/vkf-dotnet7.7z файл `service.options`.
* В частности, настройки "random at start folder", закомментируйте две команды ping (если не хотите сетевой активности), измените путь "/media/veracrypt8/tmp/entropy" на ваш путь к шифрованному постоянному файлу энтропии (либо закомментируйте символом "#" этот блок).
* Изменённый файл должен остаться в архиве.
