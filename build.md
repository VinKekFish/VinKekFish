...
* С помощью команды cd поменяйте директорию на ту, где удобно будет временно распаковать архив.
* Запустите vkf-dotnet7.sh (если под Linux установлен пакет dotnet7) или vkf-linux.sh на чистом Linux без dotnet7.
* При предложении поменять настройки при установке, поменяйте настройки в /opt/VinKekFish/options/service.options (как предлагается в скрипте, для задержки нажмите Enter). В том числе, указанные в пункте "service.options".
* Если вы меняли файл service.options уже после установки, то выполните команду ```systemctl restart vkf.service``` для перезапуска сервиса vkf. Если файл service.options был изменён в процессе установки, то этого делать не нужно.
