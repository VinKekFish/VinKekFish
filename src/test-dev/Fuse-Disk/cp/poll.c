...
// pkg-config fuse3 --cflags --libs == -I/usr/include/fuse3 -lfuse3 -lpthread
...
// Это должно быть в заголовочном файле fuse.h, но там этого нет
// https://github.com/libfuse/libfuse/blob/master/include/fuse.h#L68
...
static int vinny_getattr(const char *path, struct stat *stbuf,
			struct fuse_file_info *fi)
{
    printf("\n\nvinny_getattr\n\n");
...
static int vinny_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
			off_t offset, struct fuse_file_info *fi,
			enum fuse_readdir_flags flags)
{
    printf("\n\nvinny_readdir\n\n");
...
static int vinny_open(const char *path, struct fuse_file_info *fi)
{
    printf("\n\nvinny_open\n\n");
	int idx = 0;
...
static int vinny_release(const char *path, struct fuse_file_info *fi)
{
	int idx = fi->fh;
printf("\n\nvinny_release\n\n");
	(void) path;
...
static int vinny_read(const char *path, char *buf, size_t size, off_t offset,
		     struct fuse_file_info *fi)
{
	int idx = fi->fh;
printf("\n\nvinny_read\n\n");
...
static const struct fuse_operations vinny_fuse_operations = {
	.getattr	= vinny_getattr,
	.readdir	= vinny_readdir,
	.open		= vinny_open,
	.release	= vinny_release,
	.read		= vinny_read
};
...
int main(int argc, char *argv[])
{
    ...
    // char buffer[16];
    // snprintf(buffer, sizeof(buffer), "%lu", (unsigned long)sizeof(fsel_oper));
    
    // struct fuse_config fuse_config_s;
