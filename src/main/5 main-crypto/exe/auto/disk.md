...
Создание диска (первое монтирование) также требует, чтобы сервис энтропии `vkf` работал (работают пути `/dev/vkf/cranom`). Если скрипт `install.sh` завершился успешно, скорее всего, сервис энтропии работает.
