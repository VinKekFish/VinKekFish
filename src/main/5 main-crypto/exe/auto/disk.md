...
###### blocksize
Размер блока файловой системы. 12 - 4096 байтов (4 Киб). 16 - 65536 байтов (64 Киб). 19 - 0.5 Миб.
Размер по умолчанию: 16 (64 Киб).
Размер блока - это размер файлов, на который разбит диск. Чем больше блок, тем меньше злоумышленник, читающий в режиме реального времени шифрованный диск, знает о том, сколько байтов было записано в какой блок. Однако, это привносит проблемы с перезаписью большого объёма информации, которая, возможно, не подвергалась изменениям.

Размер 12 может применяться тогда, когда нужно снизить количество измененй на диске при каких-то малых изменениях. Работает очень долго (в том числе, долго форматирует диск) и снижает скрытнось размеров изменений данных на дисках (если вы изменили 1 байт при размере 16 - злоумышленник не знает, изменили ли вы 65536 байтов или всего 1, а при размере 12 злоумышленник знает, что изменилось не более 4096 байтов).
Размер 12 работает очень медленно из-за наличия файловой транзакции: при записи на диск хотя бы 1 байта создаётся несколько много файлов, а при записи крупных блоков записей на диск вспомогательных файлов становится очень много.
...
Учтите, что при копировании в таком режиме вообще может не начаться копирование. Оно начнётся только при операции размонтирования и может продолжаться очень долго (часами).

В целом, такая операция скорее не рекомендуется. Иногда она интересна при копировании большого количества мелких файлов (сильно убыстряет копирование).
После того, как вы совершили операцию, но перед размонтированием системы, рекомендуется снова установить синхронную запись командой `mount -o remount,sync путь` (ожидайте её завершения), так как при большом количестве записанных данных сброс их на диск может продолжаться часами. Вместо этого можно просто размонтировать файловую систему: программа сама ждёт сброса кеша на диск.

Запустить на шифрованном жёстком диске firefox или иной браузер на основе chromium невозможно, если не включена опция async.


## Команда unsafe
При необходимости записи большого количества информации (совместно или отдельно от опции async) можно пробовать использовать опцию `unsafe` для ускорения работы (2 раза и более). Эта опция позволяет небезопасную запись на диск. Однако, в случае сбоя (потери электропитания, например) данные могут быть утеряны, вплоть до полного отказа всей файловой системы.
Убедитесь, что:
1. Либо файловая система пуста (нет возможности потерять данные).
2. Либо данные файловой системы надёжно резервно копированы (или зашифрованная файловая система резервно скопирована).
3. Либо в наличии файловая система с дублированием данных, например, btrfs, отформатированная с ключами -m dup -d dup (это не поведение mkfs.btrfs по умолчанию). Это не очень надёжно, так как теоретически могут повредиться сразу несколько блоков файловой системы, так как btrfs может иметь меньший размер узла файловой системы, чем блок шифрования.

Введите опцию
```unsafe:yes```
в конфигурационный файл (на отдельной строке).
Смонтируйте файловую систему и сделайте запись большого количества данных. После этого уберите или закомментируйте опцию (или измените на `unsafe:false`) и перемонтируйте систему в безопасном варианте.
...
В процессе монтирования для записи всегда записываются служебные данные. Монтируя диск только для чтения мы исключаем запись на него даже служебных данных. Диск остаётся такое же, как был. В то же время, остаётся возможность вызова служебных программ, типа zerofree (для ext4).
...
```
Разумеется, в команду необходимо подставить нужный путь вместо указанного.

### Опция full-readonly
`full-readonly:yes` блокирует любую запись на эмулируемое устройство. В отличие от опции монтирования "ro" для программы mount, эта опция блокирует любую запись через систему эмуляции шифрованного диска, то если запрещён на запись даже доступ к самому устройству и команды типа "mount -o remount,rw" не помогут.


## Форматирование диска без журнала
Форматирование диска без журнала может несколько ускорить запись на диск (иногда, до двух раз), однако это может увеличить вероятность поломки файловой системы при зависаниях компьютера или потере электропитания. Это осуществляется опцией ```no-journal: yes``` при создании (первом монтировании) шифрованного диска. В целом, шифрованный диск обеспечивает надёжную запись в пределах одного блока файловой системы и критические повреждения системы при отключении питания маловероятны.
...
## Форматирование под иную файловую систему
Команда на форматирование может быть изменена с помощью опции `format-cmd` по сравнению с заданной по умолчанию. По умолчанию форматирование идёт в файловую систему ext4.

Команда по умолчанию выглядит примерно так
```format-cmd:mke2fs -t ext4 -v -b 4096 -I 1024 -N {iN} -C 64k -m 0 -O has_journal,extent,bigalloc,inline_data,flex_bg,resize_inode,sparse_super2,dir_nlink $$$dev$$$```
Где `{iN}` - это количество блоков шифрования на диске (размер блока шифрования по умолчанию 65536 байтов, ёмкость диска делим на его размер).


Пример форматирования в btrfs.
...
При монтировании btrfs, если необходимо сжатие, нужно указать как дополнительный параметр монтирования параметр, задающий алгоритм сжатия, например параметр `compress=zlib`.

Например:
```mount-o:compress=zlib```
для монтирования со сжатием zlib без других опций.
Или
```mount-o:compress=zstd:15,noexec=recursive,nosuid=recursive,nodev=recursive,nosymfollow=recursive,rw```
для монтирования со сжатием zstd с уровнем сжатия 15 и дополнительными опциями.
...
```
Мы видим путь, по которому смонтировано устройство, видим путь к логическому устройству (`/dev/loop16`), наименование типа файловой системы (`ext4`), опции монтирования (чтение-запись, одноразовые пометки времени последнего доступа). Наименование этого логического устройства указано и в логах программы
```
Started with loop device
/dev/loop16
```
