...
Учтите, что при копировании в таком режиме вообще может не начаться копирование. Оно начнётся только при операции размонтирования и может продолжаться очень долго.

**`Обратите внимание, что режим async иногда приводит к некорректному копированию файлов и скопированные файлы лучше проверять на целостность (хотя бы проверить, что совпадают размеры файлов до байта или суммарные размеры директорий при копировании). Причём, такую проверку лучше делать после перемонтирования диска.`**

В целом, такая операция скорее не рекомендуется. Иногда она интересна при копировании большого количества мелких файлов.
