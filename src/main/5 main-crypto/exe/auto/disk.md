...
Эта команда отключает синхронную передачу информации на диск. Теперь при копировании информации на диск будет казаться, что операция выполнилась очень быстро. Однако, никаких операций ещё не выполнено и файловая система "зависнет" и будет в фоновом режиме выполнять эту операцию. Это очень неудобно, но позволяет иногда сильно ускорить скорость записи (до 8-ми раз).

**!!Обратите внимание, что это иногда приводит к потере файлов при копировании и скопированные файлы лучше проверять (хотя бы проверить, что совпадают размеры файлов до байта или суммарные размеры директорий при копировании).!!**
## Асинхронные операции
