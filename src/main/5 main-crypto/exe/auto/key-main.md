...
## Предупреждение
Данная команда не может работать под системами, где невозможно работа сервиса генерации энтропии "vkf" ("vkf" - это эта же программа, только запущенная в режиме сервиса). При установке программы на Linux с установленной systemd (большинство известных дистрибутивов, кроме Gentoo, уже имеют данную службу) сервис "vkf" регистрируется автоматически и запускается при старте системы.
...
Внутренне устанавливается флаг isSimpleOutKey. Далее вызывается функция GenerateSimpleKey.
