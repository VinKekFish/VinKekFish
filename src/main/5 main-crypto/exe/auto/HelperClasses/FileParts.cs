    ...
    public virtual Approximation FullLen
    {
        get
        {
            ...
            return result;
                                                                ...
                                                                /// <summary>Поле, используемое свойством size. Не рекомендуется использовать, т.к. size автоматически производит перерасчёт адресов при изменении размера. Size указывает на примерный размер данной секции без учёта подчинённых секций (если нужен полный размер: см. FullLen).</summary>
    public Approximation _size = Approximation.Null;            /// <summary>Оценка размера. Если это не лист, то может иметь нулевой размер. Ненулевой размер у узла сдвигает подчинённые узлы на этот размер (то есть этот блок идёт перед подчинёнными узлами). Если записывается содержимое с помощью setArrayToRecord, то размер переопределяется автоматически.</summary>
