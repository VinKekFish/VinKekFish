...
/// <summary>
/// Данный класс шифрует сообщение, которое должно быть коротким.
/// В связи с тем, что синхропосылкой является текущая дата и время:
/// 1. Убедитесь, что записанная дата и время шифрования файла не являются важными данными, т.к. они будут доступны в расшифрованном виде
/// 2. Убедитесь, что вы шифруете только один файл в один момент времени (включая время перевода часов)
/// 3. Используйте на приём и на передачу разные ключи даже с одним адресатом (чтобы не быть одновременным уже с ним)
/// </summary>
public unsafe sealed partial class Enc_short_1_202510: IDisposable
{
    ...
    public readonly bool ToBase64 = true;
    public Enc_short_1_202510(AutoCrypt.DecEncCommand enc_dec_command, bool toBase64 = true)
    {
        ...
        KeyKeyStrenght = VKF_KEY_K*VinKekFishBase_etalonK1.BLOCK_SIZE + 64;
        HashLength     = (int) (64 - OIV_Length);
    ...
    public const nint OIV_Length = 8;                                                       /// <summary>Стойкость шифрования и стойкость генератора ключа (коэффициенты K в VinKekFish)</summary>
    ...
    public const int  FileAlignment  = 64;//1 << 16;
    ...
    public ProgramErrorCode Encrypt()
    {
        ...
        if (command.isDebugMode)
        {
            ...
            Console.WriteLine(L("Encryption started"));
        ...
        try
        {
            ...
            // Выделяем массив под синхропосылку и вычисляем её - это текущее время
            byte[]? _OIV = new byte[OIV_Length];
            BytesBuilder.ULongToBytes((ulong) DateTime.Now.Ticks, ref _OIV);
            using var OIV = Record.GetRecordFromBytesArray(_OIV!, RecordDebugName: "InitSpongesFirst.OIV");
            ...
            if (ToBase64)
            {
                using var buff = OIV | encFile;

                var base64 = new System.Text.UTF8Encoding().GetBytes(Convert.ToBase64String(buff));
                encF.Write(base64);
            }
            else
            {
