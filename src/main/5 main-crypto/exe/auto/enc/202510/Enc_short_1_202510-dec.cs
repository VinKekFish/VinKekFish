    ...
    public ProgramErrorCode Decrypt()
    {
        ...
        try
        {
            ...
            using (var encFileStream = File.Open(command.EncryptedFileName!.FullName, FileMode.Open, FileAccess.Read, FileShare.None))
            {
                ...
                if (ToBase64)
                {
                    string base64 = new System.Text.UTF8Encoding().GetString(encFile);
                    encFile.Dispose();
                    encFile = Record.GetRecordFromBytesArray(  Convert.FromBase64String(base64)  );
                    EncFileLength = encFile.len;
                }
            }
