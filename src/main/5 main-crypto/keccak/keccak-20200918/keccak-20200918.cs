...
public unsafe class Keccak_20200918: Keccak_base_20200918
{
    ...
    /// <param name="offset">Начальный индекс, с которого начинается xor в bytesFromFile.</param>
    public void DoXor(BytesBuilderForPointers.Record bytesFromFile, byte len, nint offset = 0)
    {
        if (len > KeccakPrime.BlockLen)
            throw new ArgumentOutOfRangeException("DoXor: len > KeccakPrime.BlockLen");
        if (len > bytesFromFile.len - offset)
        ...
        BytesBuilder.Xor(len, bytesFromFile.array + offset, st);
