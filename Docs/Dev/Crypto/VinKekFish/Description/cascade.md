...
Если пользовательские данные вводятся, то данные вводятся также по блокам 64-ре байта. Однако, из каждого блока берутся только первые 64-⌈N/wide⌉ байтов (или менее), которые вводятся в губку. То есть, обратная связь заполняет все значения, которые не были заполнены пользовательским вводом.
