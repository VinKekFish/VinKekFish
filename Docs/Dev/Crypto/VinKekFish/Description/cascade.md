...
В этом файле расположены описания дублирующих алгоритмом повышенной стойкости


## Одиночный каскад keccak
...
Для этого первое 8-мибайтовое слово губки может быть приращено на 1. Информация, попадающая на выход, не требует такого приращения. Приращение осуществляется только в обратной связи.
Вместо приращения на 1 информация может быть подвергнута шифрованию TreeFish (с приращением его tweak на единицу каждый раз). При этом недостающая часть блока равна нулю в первый раз и равна предыдущему выходному блоку в следующий раз.
Таким образом, вычисление обратной связи с блоком TreeFish следующее.
1. Инициализировать доп. ключом TreeFish. Tweak инициализировать нулём или другим значением.
...
3. Провести 2\*k\*log(k) шагов каскада (для того, чтобы обратная связь по данным, сгенерированным на шаге взятия выхода, прошла через все слои губки, с учётом возможной потери информации при взаимодействии данных).
...
4) Провести 2\*k\*log(k) шагов каскада. Перейти в шагу 4.1 или, если данные для инициализации закончились, перейти к шагу 5.
5. Ввести в режиме OVERWRITE обратную связь в верхнюю губку. Провести ещё 2\*k\*log(k) шагов каскада. Считать губку проинициализированной
...
4. После инициализации губок провести 2\*k\*log(k) шагов каскада.


В режиме генерации ключа (или в режиме шифрования небольшого количества важных данных), выход берётся следующим образом:
1. W=k.
2. Провести после предыдущего взятия выхода (или инициализации) 2\*k\*log(k) шагов каскада вхолостую (для того, чтобы обратная связь по данным, сгенерированным на шаге взятия выхода, прошла через все слои губки, с учётом возможной потери информации при взаимодействии данных).
...
1. W=2. Для режима генерации ключа с вводом рандомизирующей информации W=k.
2. Провести k шагов каскада вхолостую (для того, чтобы обратная связь по данным, сгенерированным на шаге взятия выхода, прошла через все слои губки). Для режима генерации ключа - k\*log(k) шагов.
3. Взять ⌊64/W⌋ байтов из самой нижней губки на выход каскада
4. Сделать 1 шаг каскада вхолостую. Для режима генерации ключа - k\*log(k) шагов.
...
1) В каскаде строго не менее 3-х губок.
...
4. Провести не менее чем 2\*k\*log(k) шагов каскада
5. i = 0
6. Взять выход левой губки на уровне i и поместить его в левую часть ключа ThreeFish, работающего после уровня i
7. Провести не менее чем 2\*k\*log(k) шагов каскада. Выполнить i++ (приращение i на единицу). Повторить пункт 6.
8. После прохождения всех левых губок перейти на шаг 5 и повторить шаги уже для правых губок и правых частей ключей ThreeFish. После инициализации правых частей ключей ThreeFish алгоритм проинициализирован полностью без дополнительных шагов.
