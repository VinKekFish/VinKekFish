...
Каскадная губка реализована в программе VinKekFish как дополнительный алгоритм (к алгоритму VinKekFish) и, возможно, обладает даже большей стойкостью.
...
W=1+log(k) - коэффициент запаса для таких коллизий.
...
Ввод данных осуществляется в каждую губку не более чем по Wn байтов, всего (во все губки) не более maxDataLen=Wn\*wide за один шаг каскадной губки.
Данные разбиваются для губок поровну (последние губки недополучают какое-то количество ввода, заменяя его вводом с обратной связи).
То есть, если вводится N <= maxDataLen байтов, то в каждую губку вводится ⌈N/wide⌉ байтов. В целом, это может быть Wn+1 байт или менее. В поледнюю губку вводится остаток от того, что ещё не введено (если wide>=64, то, теоретически, возможно, в последнюю губку вообще не будет никакого ввода, а в предпоследнюю будет частичный).

При вводе данных всегда делается дополнительный шаг каскадной губки, т.к. часть обратной связи губка теперь не получила (то есть меньше защита от противника), а данным нужно хорошо распространиться по всему каскаду. То есть если введён хотя бы один байт пользовательских данных, то губка делает ещё один шаг сразу же. Шаг губки при вводе данных сразу считается двойным всегда, даже если при описании это не описано.
