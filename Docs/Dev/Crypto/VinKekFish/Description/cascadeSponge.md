...
for tall in [4, 8, 16, 32, 64, 88, 176]:
...
Если ввод данных есть, то на первом шаге для обратной связи остаётся меньше места для ввода. Таким образом, обратная связь заполняет ячейки, оставшиеся незаполненными вводом. Сначала во буферный массив ввода записываются данные обратной связи. Пусть мы хотим ввести N байтов в подгубку. Тогда в буферном массиве мы перезапишем N первых байтов этими данными. При записи в подгубку 64-х байтов ввода, в эту подгубку логически записывается, что введено не 64-ре байта, а N байтов. Это связано с тем, что мы должны точно зафиксировать количество введённых байтов именно из ввода пользователя (его длина переменная), а общий ввод всегда один и тот же: 64 байта. В том числе, при вводе 0 байтов, все операции делаются точно также.
